...
Task 7 exposes context lag, counts, reconciliation age, backfill state, and bounded reason codes without making ordinary indexing degradation a readiness failure. Recovery verification validates migration/checksum integrity, SQLite/foreign-key consistency, exact FTS token positions and vector document identities, a joined four-tier query, tombstones, and schema-prefix-available backfill progress. Every supported startup replays every retained current and noncurrent content-free journal generation through an independently recorded recovery image before Discord secrets or the target runtime are available; download, shape, checksum, migration, capability, or replay failure stops startup. Append-only migration 0013 preserves valid legacy Discord text identity so a memory-only forget still scrubs a migration-0002 backup. Deploy and restore preserve image/database compatibility through an immutable image capability label, mode-0600 artifacts, and at-most-30-day local recovery retention, while bucket policy retains database artifacts for 30 days and journals for 60 days without widening runtime permissions. The repository gate passes 440 deterministic tests with 91.33% statement, 83.67% branch, 94.16% function, and 92.80% line coverage. The container image `sha256:35fe6e13ab736ad480dcef24415411dd55509b05419ef5cfd9dac77e37ee822b` advertised `0003_channel_context` and returned `READY`; both exact migration-0003 and current fixtures reported the expected capability, and the current fixture backup passed explicit verification and the restore drill. Shell syntax checks and Terraform initialization/validation passed. Milestone 8 live acceptance remains pending authorization; no live Discord, provider, deployment, or production data call is claimed.
