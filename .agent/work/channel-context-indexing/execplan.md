...
Task 7 exposes context lag, counts, reconciliation age, backfill state, and bounded reason codes without making ordinary indexing degradation a readiness failure. Recovery verification validates migration/checksum integrity, SQLite/foreign-key/vector/search consistency, tier queries, tombstones, and backfill progress. Every supported startup replays retained content-free journals through an independently recorded recovery image before Discord secrets or the target runtime are available; download, shape, checksum, migration, or replay failure stops startup. Deploy and restore preserve image/database compatibility, mode-0600 artifacts, and at-most-30-day local recovery retention, while bucket policy retains database artifacts for 30 days and journals for 60 days without widening runtime permissions. The repository gate passes 431 deterministic tests with 91.20% statement, 83.56% branch, 93.90% function, and 92.67% line coverage. The container image built and returned `READY`; an explicit migration-0003 fixture backup and restore drill, shell syntax checks, and Terraform initialization/validation passed. Milestone 8 live acceptance remains pending authorization; no live Discord, provider, deployment, or production data call is claimed.
