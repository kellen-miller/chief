...
- [x] (2026-07-14 17:17Z) Fresh-eyes recent-work review found that the global coverage gate masked three new modules below the plan's per-module 80% branch requirement. Focused label-sanitization, provider-default/error, restore-target, malformed-journal, and legacy-reason tests raised every new included `src/**/*.ts` module above 80% in statements, branches, functions, and lines. `pnpm verify` passed 49 files and 457 tests with 91.66% statement, 84.37% branch, 94.51% function, and 93.05% line coverage. The deep-module boundary remains intact: callers do not know context tables, tier ranking, or provider sequencing.
- [x] (2026-07-14 17:38Z) Formal implementation review corrections made live and reconciled Discord deletions await the exact immutable external journal upload, preserved ordinary replies when live indexing fails, added hourly host pruning and effective at-most-30-day database-artifact lifecycle rules, kept long-term inputs scoped to proposal-cited documents without blind weekly fanout, and removed Discord IDs and error payloads from logs. `pnpm verify` passed 49 files and 462 tests with 91.58% statement, 84.08% branch, 94.43% function, and 93.00% line coverage.
...
- Observation: bucket versioning makes a delete lifecycle transition a live database object to noncurrent before permanent deletion, so a 30-day live rule plus a 30-day noncurrent rule can retain recovery bytes for roughly 60 days.
  Evidence: formal implementation review reproduced the lifecycle sequence; database objects now transition at 28 days and noncurrent database generations expire after one day, while content-free forget journals retain their separate at-least-60-day policy.
- Observation: a daily topic proposal's source IDs are its relevance boundary; replacing them with the whole daily document or fanning every weekly document into every topic contaminates topic lineage.
  Evidence: the formal review regression cites one hourly child from a daily summary and proves the topic job contains only that child; weekly completion no longer changes an unrelated completed topic job.
...
- Decision: expose authoritative Discord suppression as an async durability boundary while retaining the synchronous local transaction as the shared mutation primitive.
  Rationale: the local tombstone must commit immediately, but a gateway or reconciliation handler must not report completion until the exact content-free journal is externally durable; failed upload stays suppressed and retryable while surfacing failure.
  Date/Author: 2026-07-14, Codex during formal implementation review.
- Decision: use proposal-cited daily child documents as long-term topic inputs and require a complete topic-label anchor for weekly consolidation.
  Rationale: daily proposals refresh directly cited topics, while a weekly aggregate may update only topics whose full distinctive label is present; unqualified all-topic fanout would permanently mix unrelated discussion.
  Date/Author: 2026-07-14, Codex during formal implementation review.
...
Task 8 exercises real source FTS, context vectors, durable-memory vectors, tier/lineage deduplication, temporal correction ordering, summary-only provenance, suppression, and production payload serialization across 44 pinned cases. CI requires zero forbidden claims, zero complete-surface leakage, exact returned provenance allowlists, complete requested-link recall, and correct target history-versus-memory classification. The optional owner-run evaluator uses the same assembled payload, a dedicated structured grader, per-class and macro classification thresholds, and numeric-only logs with model/timestamp evidence. After the fresh-eyes coverage correction, the final repository gate passes 457 deterministic tests with 91.66% statement, 84.37% branch, 94.51% function, and 93.05% line coverage; every new included source module independently clears 80% in all four dimensions. Repository implementation is complete; production acceptance remains pending explicit authorization for deployment, paid grading/backfill, and live Discord verification. No live Discord, provider, deployment, backfill, or production-data call is claimed.

Formal implementation review then closed five repository-acceptance gaps without widening owner-only production scope. Authoritative deletions now await immutable journal durability, ordinary requests fall back across an indexing-write failure, local and bucket database recovery bytes stay within the approved bound, topic lineage follows cited evidence, and operational logs expose no private IDs or error payloads. The corrected repository gate passes 462 deterministic tests. Production deployment, paid grading, and live backfill remain pending explicit authorization.
...
The companion domain glossary is `CONTEXT.md`, and the terms needed to execute this plan are repeated here so the plan remains self-contained. A source event is an eligible human or Chief message. Recent conversation is the short chronological timeline. Historical context is a time-bounded report of discussion and is not truth. A rollup is a derived report with source lineage. A context tier is hourly, daily, weekly, or long-term. A durable memory is an accepted fact or preference. A suppression tombstone is a content-free record that prevents deleted material from returning. FTS5 is SQLite's built-in full-text search index; this repository uses `conversation_event_fts`, `context_document_fts`, and `memory_fts` to find retained text lexically. sqlite-vec is the SQLite extension that stores and searches fixed-length embedding vectors in `context_document_vectors` and `memory_vectors`. A Discord snowflake is the 17-20 digit globally unique identifier Discord assigns to a message, channel, guild, or other object; source scopes combine the configured guild snowflake, channel snowflake, and message snowflake. A checksum is the SHA-256 digest stored beside a migration, source revision, journal, or manifest so changed content or identity can be detected without retaining another copy. Reconciliation is a bounded Discord history scan that compares remote message identities and revisions with the local canonical ledger to recover missed creates, edits, or deletions after downtime.
...
Use tight red-green loops for each agreed seam:
    ...
    pnpm exec vitest run --project integration test/integration/context-rollup.test.ts
    ...
    pnpm exec vitest run --project integration test/integration/channel-context-service.test.ts
...
- 2026-07-14: Fresh-eyes review closed the plan's per-module coverage gap with executable provider, payload-sanitization, and recovery error/default tests. All new included source modules now clear 80% statements, branches, functions, and lines without adding exclusions or changing production behavior.
- 2026-07-14: Formal implementation review closed deletion-durability, reply-availability, recovery-retention, topic-lineage, and log-privacy gaps with focused regressions and a green 462-test repository gate. Standards review also corrected stale test filenames and made the plan's storage/recovery terminology self-contained; low-risk historical commit wrapping and helper-duplication observations remain recorded rather than rewriting published implementation history or forcing an unrelated refactor.
