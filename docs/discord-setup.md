...
2. Enable the privileged **Message Content Intent**. Chief also uses the ordinary Guilds, Guild Messages, Guild Voice States intents.
...
Commands are guild-scoped so updates appear quickly. Chief fails closed if the configured guild or either channel is missing, if a command is invoked elsewhere, or if a message comes from a DM, thread, webhook, or bot.
